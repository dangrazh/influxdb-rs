...
async fn reduced_perms() {
    ...
    assert!(
        create_auth.is_ok(),
        "CREATE AUTH DIDNT WORK: {} ORG ID: {} AuthType: {}",
        create_auth.unwrap_err(),
        client.org_id,
        AuthResourceType::Bucket.to_string()
    );
    ...
    assert!(
        reduced_client.is_err(),
        "REDUCED CLIENT DIDNT WORK: {}",
        reduced_client.unwrap_err()
    );

    // Access Bucket with reduced client that doesnt have org ID access
    let reduced_client = Client::new_without_org_id(
        Url::parse("http://localhost:8086").unwrap(),
        "test_bucket",
        "test_org",
        &auth.token,
    ...
    assert!(
        result.is_ok(),
        "REDUCED CLIENT QUERY DIDNT WORK: {}",
        result.unwrap_err()
    );
}
