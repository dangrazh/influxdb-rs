...
async fn users() {
    ...
    assert!(
        list_users.is_ok(),
        "LIST USERS DIDNT WORK: {}",
        list_users.unwrap_err()
    );
    ...
    for user in users {
        if user.name == "reduced_perms" {
            ...
            assert!(
                delete_user.is_ok(),
                "DELETE USER DIDNT WORK: {}",
                delete_user.unwrap_err()
            );
        }
    }
    ...
    let create_user = client
        .create_new_user("reduced_perms", Status::Active)
        .await;

    // Assert that it succeeded
    assert!(
        create_user.is_ok(),
        "CREATE USER DIDNT WORK: {}",
        create_user.unwrap_err()
    );

    // Get Create User Response
    let _user = create_user.unwrap();

    //Create Read Permissions
    let _permissions = vec![AuthPermissions {
