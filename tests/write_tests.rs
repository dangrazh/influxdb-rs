...
async fn create_and_delete_measurement() {
    ...
    let drop = client
        .drop_measurement("temporary", &now.to_rfc3339(), &later)
        .await;
...
async fn use_points() {
    ...
    let drop1 = client
        .drop_measurement("test1", &now.to_rfc3339(), &later)
        .await;

    assert_eq!(drop1.is_ok(), true);

    let drop2 = client
        .drop_measurement("test2", &now.to_rfc3339(), &later)
        .await;
...
async fn query_with_macros() {
    ...
    let drop = client
        .drop_measurement("test4", &now.to_rfc3339(), &later.to_rfc3339())
        .await;
